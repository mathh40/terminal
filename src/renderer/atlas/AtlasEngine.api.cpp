...
void AtlasEngine::_resolveFontMetrics(const wchar_t* requestedFaceName, const FontInfoDesired& fontInfoDesired, FontInfo& fontInfo, FontMetrics* fontMetrics) const
{
    ...
    const auto adjustedWidth = std::roundf(fontInfoDesired.GetCellSizeAdjustmentX().AdjustValue(advanceWidth, _api.dpi));
    const auto adjustedHeight = std::roundf(fontInfoDesired.GetCellSizeAdjustmentY().AdjustValue(advanceHeight, _api.dpi));
